...
gradedReesPiece(ZZ, Ideal) := opts -> (n1, J1) -> (
    ...
    if (S1#"ExtendedReesAlgebra" == true) then (
        if not isHomogeneous J1 then error "gradedReesPiece:  Expected a homogeneous ideal or a Rees piece";
        R1 := S1#"BaseRing";
        ...
        degList := apply(genList, zz->first (degree zz) );
        baseGens := S1#"BaseRingList";
        badMap := map(R1, S1, (gens R1) | baseGens);
        tempGens := ideal(0_R1);
        i := 0;
        while (i < #genList) do (
            if (degList#i == n1) then (
...
testIdealNP(QQ, Ideal) := opts -> (n1, I1) -> (
    ...
    local degShift;
    local S1;
    if (floor n1 == n1) then (--integer, gradedReesPiece does not work with classicalReesAlgebra right now
        ...
        return gradedReesPiece(floor n1, tauOmegaSList#0);
    )
